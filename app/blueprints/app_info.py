...
@app_info.route("/app-name", methods=["GET", "OPTIONS"])
def get_app_name() -> Response:
    """Returns the IP address. The HTTP Response object contains JSON formatted as:

    {
        "app-name": <app name and version>
    }
    """
    if request.method == "OPTIONS":
        return cors_utils.get_http_response_for_options_request(request)
    return json_utils.json_response(
        {"app-name": f"{env.app_name} version {env.app_version}"},
...
def test_exception() -> Response:
    """Raises ValueError to test the Flask exception handling and logging."""
    raise ValueError("Testing exception handling")


@app_info.route("/teapot", methods=["GET", "OPTIONS"])
def test_teapot() -> Response:
    """Raises ImATeapot to test the Flask HTML template for HTTP error 418.
...
def test_unauthorized() -> Response:
    """Raises an Unauthorized error to test the Flask HTML template for HTTP error 401."""
    raise Unauthorized()
